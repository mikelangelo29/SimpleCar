...
class AmmortizzatoriScreen(MDScreen):
    """
    Screen Ammortizzatori — stesso schema/grafica di Dischi/Pastiglie:
    ...
    def _ensure_structure(self):
        ...
        am = self.current_auto.get("ammortizzatori")
        if not isinstance(am, dict):
            am = {}

        # Radio "medie" usura (60 / 80 / 100k). Default: 80k
        am.setdefault("intervallo_km", 80000)
        am.setdefault("ultimo_km", None)
        am.setdefault("prossimo_km", None)
        am.setdefault("data", "")

        self.current_auto["ammortizzatori"] = am

        # Salvataggio soft (come gli altri)
    ...
    def _build_ui(self):
        ...
        try:
            img = Image(
                source="app/assets/icons/ammortizzatori.png",
                size_hint=(None, None),
                width=dp(170),
                height=dp(170),
                allow_stretch=True,
        ...
        container.add_widget(
            MDBoxLayout(size_hint=(1, None), height=dp(10))
        )
    ...
    def _radio_row(self, label, value):
        ...
        cb = MDCheckbox(group="ammortizzatori_interval")
    ...
    def _load_values(self):
        ...
        am = self.current_auto.get("ammortizzatori", {}) or {}
        self.intervallo_km = int(am.get("intervallo_km", 80000) or 80000)

        ultimo = am.get("ultimo_km", None)
        self.field_ultimo.text = "" if ultimo is None else str(ultimo)

        self.field_data.text = (am.get("data", "") or "").strip()
    ...
    def _update_view(self):
        ...
        ultimo = int(txt)
        prossimo = ultimo + int(getattr(self, "intervallo_km", 80000))
        self.lbl_prossimo.text = f"Prossima sostituzione: {prossimo} km"

    # =========================================================
    # SALVA (blocca specifico + scadenze + JSON)
    # =========================================================
    def _save_values(self):
        ...
        ultimo_km = int(txt)
        intervallo = int(getattr(self, "intervallo_km", 80000))
        ...
        # Blocco specifico
        self.current_auto.setdefault("ammortizzatori", {})
        self.current_auto["ammortizzatori"]["intervallo_km"] = intervallo
        self.current_auto["ammortizzatori"]["ultimo_km"] = ultimo_km
        self.current_auto["ammortizzatori"]["prossimo_km"] = prossimo_km
        self.current_auto["ammortizzatori"]["data"] = data_txt
    ...
    def _show_info(self):
        dialog = MDDialog(
            ...
            text=(
                "Indica dopo quanti chilometri, in media, gli ammortizzatori "
                "andrebbero controllati o sostituiti.\n\n"
                "Valore più basso = molta città, buche, strade sconnesse.\n"
